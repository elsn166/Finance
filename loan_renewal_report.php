  ...
  <div class="container-fluid">
    ...
    <div class="card card-outline card-success">
      <div class="card-header">
        <h3 class="card-title">Loan Renewal Report</h3>
             <div>
  <a class="btn-sm btn-success float-right" href="loan_renewal_report_print.php?Startdate=<?php echo $from_date; ?>&enddate=<?php echo $to_date;?>&branch_id=<?php echo $branch_id;?>" target="_blank">Print</a>
  </div>
      </div>
 ...
 if($session_role_id == 1 || $session_role_id == 2 || $session_role_id == 9)
{
  ...
  $renewal_list=select_data(LOAN_RENEWAL," where customer_id in (select customer_id from ".CUSTOMER_MASTER." where branch_id='".$branch_id."') and loan_renewal_date >= '".$from_date."' and  loan_renewal_date <= '".$to_date."' order by loan_renewal_date asc");
}elseif(($from_date!='') && ($to_date!='')){
         $renewal_list=select_data(LOAN_RENEWAL," where loan_renewal_date >= '".$from_date."' and  loan_renewal_date <= '".$to_date."' order by loan_renewal_date asc");
}elseif(($branch_id!='')){
      $renewal_list=select_data(LOAN_RENEWAL," where customer_id in (select customer_id from ".CUSTOMER_MASTER." where branch_id='".$branch_id."') order by loan_renewal_date asc");
}
}
  else{

    $renewal_list=select_data(LOAN_RENEWAL," where customer_id  in (select customer_id from ".CUSTOMER_MASTER." where branch_id='".$session_branch_id."') and loan_renewal_date >= '".$from_date."' and  loan_renewal_date <= '".$to_date."' order by loan_renewal_date asc");
  }
...
if(count($renewal_list)>0)
{
    ...
    {
      $tot_amt = $tot_amt + $cl['loan_renewal_amt'];
    }
}
...
if($session_role_id == 1 || $session_role_id == 2 || $session_role_id == 9)
{
  ...
  $renewal_list=select_data(LOAN_RENEWAL," where customer_id in (select customer_id from ".CUSTOMER_MASTER." where branch_id='".$branch_id."') and loan_renewal_date >= '".$from_date."' and  loan_renewal_date <= '".$to_date."' order by loan_renewal_date asc");
      ...
      $renewal_list=select_data(LOAN_RENEWAL," where customer_id in (select customer_id from ".CUSTOMER_MASTER." where branch_id='".$branch_id."')' order by loan_renewal_date DESC");
}
}
else
{
...
if(count($renewal_list)>0)
{
    ...
    foreach($renewal_list as $cl)
    {
      ...
      $date = date('d-m-Y', strtotime($cl['loan_renewal_date']));
      $loan_amt = $cl['loan_renewal_amt'];
      
      $member_id = $cl['customer_id'];
      $loan_id =  $cl['loan_id'];
      $member_name = customer_name($member_id);
      $member_no = customer_no($member_id);
      $loan_no = loan_no($loan_id);
?>
