...
{
	global $CN;
	$select = mysqli_query($CN,"SELECT ref_loan_no FROM ".LOAN_MASTER." where loan_type_id='$loan_type_id' and branch_id='$session_branch_id' and loan_id!='$loanid' and loan_no!='' ORDER BY loan_id DESC LIMIT 1");
...
function loan_no($loan_id){
	global $CN;
	$select = mysqli_query($CN,"SELECT loan_no FROM ".LOAN_MASTER." WHERE loan_id = '$loan_id' and loan_no is not null");
	$rowcount=mysqli_num_rows($select);
	if($rowcount > 0)
	{
	if($row = mysqli_fetch_array($select)){
...
//  newly added madurai branch received ammount
function madurai_received($date)
{
	global $CN;
	$select = mysqli_query($CN,"SELECT sum(expense_amount) as expense_amount FROM ".EXPENSE_MASTER." WHERE expense_type_id = 44 and status=2 and expense_date='$date'");
...
// newly added madurai branch received ammount end
//remitted ammount madurai branch start
function madurai_remitted($date)
{
	global $CN;
	$select = mysqli_query($CN,"SELECT sum(expense_amount) as expense_amount FROM ".EXPENSE_MASTER." WHERE expense_type_id = 45 and status=2 and expense_date='$date'");
...
// remitted ammount madurai branch end
...
// madurai branch crm received ammount

function madurai_received_branch($date,$session_branch_id)
{
	global $CN;
	$select = mysqli_query($CN,"SELECT expense_amount FROM ".EXPENSE_MASTER." WHERE expense_type_id = 44 and status=2 and expense_date='$date' and branch_id='$session_branch_id'");
...
//madurai branch crm received ammount end
...
// madurai remitted ammout for crm start
function madurai_remitted_branch($date,$session_branch_id)
{
	global $CN;
	$select = mysqli_query($CN,"SELECT expense_amount FROM ".EXPENSE_MASTER." WHERE expense_type_id = 45 and status=2 and expense_date='$date' and branch_id='$session_branch_id'");
...
//madurai remitted ammount for crm end
...
function send_sms_olddddd($mob_no,$member_no,$customer_name)
{
// Account details
...
$message = rawurlencode('அன்புள்ள வாடிக்கையாளர் "'.$customer_name.'" அவர்களே!! அமுதினி நிறுவனத்திற்கு தங்களை அன்புடன் வரவேற்கின்றோம். தங்களது வாடிக்கையாளர் எண்:"'.$member_no.'".எங்களுடன் இணைந்ததற்கு நன்றி.');
...
{
// Account details
$apiKey = urlencode('NTg2NDY1NTE1OTQ3MzM3YTZjNGYzNzc2NjIzNjQ4NmM=');
...
// account creation
function send_sms_account_creation($mob_no,$acc_no,$customer_name,$plan_code)
{
// Account details
$apiKey = urlencode('NTg2NDY1NTE1OTQ3MzM3YTZjNGYzNzc2NjIzNjQ4NmM=');
...
$message = rawurlencode('அன்புள்ள வாடிக்கையாளர் '.$customer_name.' அவர்களே!!.அமுதினி நிறுவனத்திற்கு தங்களை அன்புடன் வரவேற்கின்றோம்.நீங்கள் '.$plan_code.' என்ற திட்டத்தை தேர்ந்தெடுத்து உள்ளீர்கள்.தங்களுக்கு வழங்கப்பட்டுள்ள சேமிப்பு கணக்கு எண்:'.$acc_no.'');
...
// account renewal
function send_sms_account_renewal($mob_no,$acc_no,$customer_name,$date,$amt,$total_amt)
{
// Account details
$apiKey = urlencode('NTg2NDY1NTE1OTQ3MzM3YTZjNGYzNzc2NjIzNjQ4NmM=');
...
$message = rawurlencode('அன்புள்ள வாடிக்கையாளர் '.$customer_name.' அவர்களே!!.தங்களுடைய சேமிப்புக் கணக்கு எண்:'.$acc_no.' - இல் '.$date.' அன்று Rs.'.$amt.' வரவு வைக்கப்பட்டுள்ளது. இன்றுவரை தங்கள் சேமிப்புக் கணக்கில் Rs.'.$total_amt.' உள்ளது. தாங்கள் அமுதினி நிறுவனத்தை தேர்ந்தெடுத்தமைக்கு நன்றி');
...
// account close
function send_sms_account_close($mob_no,$acc_no,$customer_name,$date)
{
// Account details
$apiKey = urlencode('NTg2NDY1NTE1OTQ3MzM3YTZjNGYzNzc2NjIzNjQ4NmM=');
...
$message = rawurlencode('அன்புள்ள வாடிக்கையாளர் '.$customer_name.' அவர்களே!!.தங்களுடைய சேமிப்புக் கணக்கு எண்:'.$acc_no.', '.$date.' அன்று நிறைவடைந்தது. தாங்கள் அமுதினி நிறுவனத்தை தேர்ந்தெடுத்தமைக்கு நன்றி.');
