...
if(isset($_POST['update']))
{
  ...
  // generating account number only for the first time
   ...
   if($acctnumber == '')
   {
    ...
    if($session_role_id==1)
    {
      $session_branch_id = $accountlist[0]['branch_id'];
    }
...
if ($plan_term=='3' && $plan_term_value =='Y' && $paln_code='D3') {
    $days="1095";
}else
if ($plan_term=='2' && $plan_term_value =='Y' && $paln_code='D2') {
    $days="730";
}else
if ($plan_term=='1' && $plan_term_value =='Y' && $paln_code='D1') {
    $days="365";
}else
if ($plan_term=='6' && $plan_term_value =='M'&& $paln_code='FM') {
    $days="26";
}else  
if ($plan_term=='180' && $plan_term_value =='D' && $paln_code='DM') {
    $days="180";
}else
if ($plan_term=='100' && $plan_term_value =='D'&& $paln_code='DH') {
    $days="1";
}else
if ($plan_term=='1' && $plan_term_value =='Y' && $paln_code='W1') {
    $days="52";
}else
if ($plan_term=='1' && $plan_term_value =='Y' && $paln_code='R1') {
    $days="12";
}else
if ($plan_term=='2' && $plan_term_value =='Y'&& $paln_code='R2') {
    $days="24";
}else
...
if($plan_id==4 && $plan_term==1 && $plan_term_value =='Y' && $paln_code='F1' )
{
$num_str1 = "$savings_renewal_amt";
$num_str2 = "$plan_intt";
$num1=$num_str2/100;
$num_1=$num_str1*$num1;
}else if($plan_id==4 && $plan_term==2 && $plan_term_value =='Y' && $paln_code='F2'){
  $num_str1 = "$savings_renewal_amt";
  $num_str2 = "$plan_intt";
  $num1=$num_str2/100;
  $num_2=$num_str1*$num1;//9500
  $num_1=$num_2*2;
  $num_4=$num_1+$num_str1;
}elseif($plan_id==4 && $plan_term==6 && $plan_term_value =='M' && $paln_code='FM'){
    $num_str1 = "$savings_renewal_amt";
$num_str2 = "$plan_intt";
$num1=$num_str2/100;
$num_1=$num_str1*$num1;
  ...
  $num_1=0;
  // $num_str2 = "$plan_intt";
 // $num_2=$num_str1*$num1;//9000 //1
 // $num_3=$num_2+$num_str1;
 // $num_4=$num_3*$num1;//2
 //
}
...
$num_1=$num_str1*$num2;
     ...
     $insert=insert_data(SAVINGS_RENEWAL,$savdata); 
      ...
      if(count($get_tally_details )>0)
      {
        ...
        $naccamt = (int)$accamt+$new_acc_amt;
      ...
      $plan_code = $plan_code.'/'.$account_amt;
       // sms sending function
       if($cus_branch_id==4 || $cus_branch_id==3 || $cus_branch_id==2 || $cus_branch_id==1)
       {
           if($status='yes' || $status=''){
           $send_sms = send_sms_account_creation($mob_no,$acc_no,$customer_name,$plan_code);
           }
       }
