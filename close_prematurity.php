...
if(isset($_GET['account_id']) && $_GET['account_id']!="" )
{
  ...
  $deduction_amt = $account_details[0]['deduction_amt'];
...
if(isset($_POST['update']))
{
  ...
  $deduction_amt = $_POST['deduction_amt'];
  ...
  $data['prematurity_acct']=1;
  $data['deduction_amt']=$deduction_amt;
  ...
  { 
    ...
    if($session_role_id == 1)
    {
      ...
      // fetch branch id from account table
      $acct_det = select_data(ACCOUNT_MASTER,"where account_id='".$_GET['account_id']."'");
      $acct_branch_id = $acct_det[0]['branch_id'];
      $acc_no = $acct_det[0]['account_no'];
      $customer_id = $acct_det[0]['customer_id'];

      $cust_details = select_data(CUSTOMER_MASTER,"where customer_id='".$customer_id."'");
      $mob_no = $cust_details[0]['mobile_number'];
      $customer_name = $cust_details[0]['customer_name'];

      //check whether for account close date entry exists in tally list table
      $get_tally_details = select_data(TALLY_MASTER,"where date='".$acd."' and branch_id='".$acct_branch_id."'");
      if(count($get_tally_details )>0)
      {
        $exp_details=select_data(TALLY_MASTER,"where date='".$acd."' and branch_id='".$acct_branch_id."'");
        $mamt=$exp_details[0]['prematurity_amt'];

        $dedamt = $exp_details[0]['deduction_amt'];


        $nmamt=(int)$mamt+$maturity_amt;

        $ndedamt = (int)$dedamt+$deduction_amt;

        $update_tallyqry="UPDATE ".TALLY_MASTER." set prematurity_amt='$nmamt',deduction_amt='$ndedamt' where date='$acd' and branch_id='$acct_branch_id'";
        $updatetally = mysqli_query($CN,$update_tallyqry);

      }
      else{

        $expdata['prematurity_amt']=$maturity_amt;
        $expdata['deduction_amt']=$deduction_amt;
        $expdata['date'] = $acd;
        $expdata['branch_id'] = $acct_branch_id;
      ...
      // $send_sms = send_sms_account_close($mob_no,$acc_no,$customer_name,$acd);


      echo "<script type='text/javascript'>window.location='view_prematurity.php?success=Account Closed Successfully';</script>";

    }
    else{

      echo "<script type='text/javascript'>window.location='view_prematurity.php?success=PreMaturity Account Details Forwarded Successfully';</script>";
  ...
  <div class="container-fluid">
    ...
    <div class="card card-outline card-success">
      <div class="card-header">
        <h3 class="card-title">PreMaturity Details</h3>
      ...
      <form method="post" action="" id="form1" name="form1">
        ...
        <div class="form-group row">
          ...
          <div class="form-group row">
            <label for="maturity_amt" class="col-sm-2 col-form-label">Deduction Amount<span style="color:red">*</span></label>
            <div class="col-sm-5">
              <input type="text" name="deduction_amt" class="form-control" id="deduction_amt" placeholder="Enter Deduction Amount" value="<?php echo $deduction_amt; ?>"/>
              <span class="has-error"></span>
            </div>
          </div>
...
maturity_amt: {required: 'Please Enter Total Amount' },
 ...
 deduction_amt: {required: 'Please Enter Deduction Amount'},
